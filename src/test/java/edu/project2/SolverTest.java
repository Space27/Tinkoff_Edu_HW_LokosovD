...
public class SolverTest {
    ...
    void breadthFirstSearch_ShouldSolveSquareMazeWithOddWalls(int width) {
        MazeGenerator generator = new PrimGenerator();
        Solver solver = new BreadthFirstSearch();
        Maze maze = generator.generate(width, width);

        List<Coordinate> coordinates = solver.solve(maze, new Coordinate(1, 1), new Coordinate(width - 2, width - 2));

        assertThat(coordinates)
            .isNotEmpty()
            .hasSizeGreaterThan(2 * (width - 3))
            .contains(new Coordinate(1, 1))
            .contains(new Coordinate(width - 2, width - 2));
    }

    @ParameterizedTest
    @ValueSource(ints = {3, 5, 11, 17, 21, 31, 51})
    @DisplayName("Квадратный лабиринт с нечетным размером для поиска в глубину")
    void depthFirstSearch_ShouldSolveSquareMazeWithOddWalls(int width) {
        MazeGenerator generator = new PrimGenerator();
    ...
    void depthFirstSearch_ShouldSolveSquareMazeWithOddWallsLikeBreadthFirstSearchInPerfectMaze(int width) {
        MazeGenerator generator = new PrimGenerator();
        ...
        List<Coordinate> depthCoordinates =
            depthSolver.solve(maze, new Coordinate(1, 1), new Coordinate(width - 2, width - 2));
        List<Coordinate> breadthCoordinates =
            breadthSolver.solve(maze, new Coordinate(1, 1), new Coordinate(width - 2, width - 2));
    ...
    void breadthFirstSearch_ShouldSolveSquareMazeWithEvenWalls(int width) {
        MazeGenerator generator = new PrimGenerator();
        Solver solver = new BreadthFirstSearch();
        Maze maze = generator.generate(width, width);

        List<Coordinate> coordinates = solver.solve(maze, new Coordinate(1, 1), new Coordinate(width - 1, width - 1));

        assertThat(coordinates)
            .isNotEmpty()
            .hasSizeGreaterThan(2 * (width - 3))
            .contains(new Coordinate(1, 1))
            .contains(new Coordinate(width - 1, width - 1));
    }

    @ParameterizedTest
    @ValueSource(ints = {4, 10, 16, 20, 30, 50})
    @DisplayName("Квадратный лабиринт с нечетным размером для поиска в глубину")
    void depthFirstSearch_ShouldSolveSquareMazeWithEvenWalls(int width) {
        MazeGenerator generator = new PrimGenerator();
        Solver solver = new DepthFirstSearch();
        Maze maze = generator.generate(width, width);
    ...
    void depthFirstSearch_ShouldSolveSquareMazeWithEvenWallsLikeBreadthFirstSearchInPerfectMaze(int width) {
        MazeGenerator generator = new PrimGenerator();
        Solver depthSolver = new DepthFirstSearch();
        Solver breadthSolver = new BreadthFirstSearch();
        Maze maze = generator.generate(width, width);
        ...
        List<Coordinate> depthCoordinates =
            depthSolver.solve(maze, new Coordinate(1, 1), new Coordinate(width - 1, width - 1));
        List<Coordinate> breadthCoordinates =
            breadthSolver.solve(maze, new Coordinate(1, 1), new Coordinate(width - 1, width - 1));
